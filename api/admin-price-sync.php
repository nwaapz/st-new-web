...
function admin_price_sync_json_payload(array $status, bool $ok = true): array
...
try {
    ...
    if ($method === 'GET') {
        ...
        api_json(admin_price_sync_json_payload($status));
    ...
    if ($method === 'PUT') {
        ...
        $sheetUrl = trim((string) ($body['sheet_url'] ?? ''));

        if ($sheetUrl === '') {

            cms_setting_set(PRICE_IMPORT_SETTING_SHEET_URL, '');
        ...
        if (stripos($sheetUrl, 'docs.google.com/spreadsheets') === false) {

            api_error('آدرس Google Sheet معتبر نیست — لینک اشتراک‌گذاری Google Sheets را وارد کنید', 400);
    ...
    if (!$status['sheet_configured']) {
        ...
        api_error('آدرس Google Sheet در تنظیمات ثبت نشده است', 400);
    ...
    api_json(array_merge(admin_price_sync_json_payload(price_import_get_sync_status()), [
